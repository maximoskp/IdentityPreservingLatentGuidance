...
from generate_utils import get_actisteer_guidance

batch_size = 4

def compute_unique_logit_activations(harmony_gt, foreign_ids, logits, threshold=0.8):
    """
...
def evaluate_actisteer_convergence(
        transformer_model,
        home_dataset,
        foreign_dataset,
        logits_loss_fn,
        ...
        for i in range(30):
            ...
            with tqdm(home_loader, unit='batch', position=tqdm_position) as tepoch:
                ...
                for batch in tepoch:
                    ...
                    home_guidance_layers = {k: home_guidance[k] for k in layers_to_steer}
                    foreign_batch = next(iter(foreign_loader))
                    mixed_batch_1 = make_mixed_batch(foreign_batch, "latent")
                    # foreign_guidance = mixed_batch_1["latent"].to(device)
...
#  end evaluate_actisteer_convergence